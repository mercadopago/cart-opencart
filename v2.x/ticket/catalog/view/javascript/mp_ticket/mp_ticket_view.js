    (function(){
        ...
        $.ajax({
            ...
            success: function success(data){
             ...
             if(i == 1)
             {
              document.getElementById('payment_method_id').value = response.methods[0].method;
              spinner.stop();
              return; 
             }
    ...
    document.getElementById('button_pay').addEventListener('click', function () {
        if(validate()){
            ...
            var url_site = window.location.href.split('index.php')[0];
            var url_backend = url_site.slice(-1) == '/' ? url_site : url_site + '/';        
            url_backend += 'index.php?route=payment/mp_ticket/payment/&payment_method_id=' + document.getElementById('payment_method_id').value;         
