...
def fill_admin_data():
    url = 'http://localhost:8888/opencart2/admin/index.php?route=payment/mp_ticket&token=%s' % utils['token']
    url_redirect = 'admin/index.php?route=extension/payment&token=%s' % utils['token']
    browser.visit(url)

    if browser.find_by_name('mp_ticketpublic_key').first.visible:
...
def check_if_admin_data_was_saved():
    admin_url = 'http://localhost:8888/opencart2/admin/index.php?route=payment/mp_ticket&token=%s' % utils['token']
