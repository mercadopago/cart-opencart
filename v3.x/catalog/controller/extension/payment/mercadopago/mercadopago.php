...
class MP {
	...
	public function set_email_admin( $email ) {
		MPRestClient::$email_admin = $email; 
	}

	public function set_country_initial( $country ) {
	...
	*/
	...
	public function get_payment_methods( $country_id ) {
		$request = array(
	...
	*/
	public function search_payment( $filters, $offset = 0, $limit = 0 ) {
		...
		$request = array(
			...
			'params' => array_merge( $filters, array( 'access_token' => $this->get_access_token() ) ),
		);
	...
	*/
	public function create_payment( $payment ) {
		...
		$request = array(
			...
			'headers' => array( 'x-tracking-id' => 'platform:v1-whitelabel,type:OpenCart3,so:' . MP::version ),
			'data' => $payment
		);
	...
	*/
	public function create_preference( $preference ) {
		$header = array( 'user-agent' => 'platform:desktop,type:OpenCart3,so:' . MP::version );
		$request = array(
...
class MPRestClient {
	...
	const MP_API_BASE_URL = 'https://api.mercadolibre.com';
			...
			if ( $json_content ) {
				...
				if ( function_exists( 'json_last_error' ) ) {
					...
					if ( $json_error != JSON_ERROR_NONE ) {
						throw new MercadoPagoException( 'JSON Error ' . json_encode( $json_error ) . ' - Data: ' . $request['data'] );
					}
				}
			} elseif ( $form_content ) {
	...
	private static function exec( $request ) {
		...
		if ( $response != null && $response['status'] >= 400 && self::$check_loop == 0 ) {
			try {
				...
				self::send_error_log( $response['status'], $errors );
			} catch ( Exception $e ) {
				throw new MercadoPagoException( 'error to call API LOGS' . json_encode( $e ) );
			}
		}
		self::$check_loop = 0;
		curl_close( $connect );
	...
	public static function send_error_log( $code, $errors ) {
		$data = array(
			'code' => $code,
			'module' => 'Opencart',
			'module_version' => MP::oc_version,
