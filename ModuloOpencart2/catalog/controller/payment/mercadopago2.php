...
class ControllerPaymentMercadopago2 extends Controller
{
    ...
    public function index() 
    {
       ...
       // "cost" => round(floatval($this->session->data['shipping_method']['cost']), 2),
        ...
        if($exclude != '') {
            ...
            error_log('accepted_methods');
            error_log(json_encode($accepted_methods));
            $all_payment_methods = $this->getMethods($country_id);
            error_log('all_payment_methods');
            error_log(json_encode($all_payment_methods));
    ...
    public function getCardIssuers()
    {
        ...
        $token = $this->config->get('mercadopago2_access_token');
    ...
    public function payment()
    {
        ...
        try 
        {
          ...
          /*  if (!in_array($payment_method_id, $accepted_methods)) {
            ...
            if($this->config->get("mercadopago2_country") != "MLM")
            {
        ...
        "street_number" => "-"));
            ...
            "notification_url" =>  $order_info['store_url'] . 'index.php?route=payment/mercadopago2/notifications', 
            ...
            {
                $payment_data["sponsor_id"] = $this->sponsors[$this->config->get('mercadopago2_country')];
            }
    ...
    private function retornoTransparente()
    {
        ...
        {
            case 'approved':
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mercadopago2_order_status_id_completed'), date('d/m/Y h:i') . ' - ' . $payment['payment_method_id'] . ' - ' . $payment['transaction_details']['net_received_amount']);
                break;
            case 'pending':
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mercadopago2_order_status_id_pending'), date('d/m/Y h:i') . ' - ' . $payment['payment_method_id'] . ' - ' . $payment['transaction_details']['net_received_amount']);
                break;    
            case 'in_process':
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mercadopago2_order_status_id_process'), date('d/m/Y h:i') . ' - ' . $payment['payment_method_id'] . ' - ' . $payment['transaction_details']['net_received_amount']);
                break;    
            case 'reject':
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mercadopago2_order_status_id_rejected'), date('d/m/Y h:i') . ' - ' . $payment['payment_method_id'] . ' - ' . $payment['transaction_details']['net_received_amount']);
                break;    
            case 'refunded':
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mercadopago2_order_status_id_refunded'), date('d/m/Y h:i') . ' - ' . $payment['payment_method_id'] . ' - ' . $payment['transaction_details']['net_received_amount']);
                break;    
            case 'cancelled':
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mercadopago2_order_status_id_cancelled'), date('d/m/Y h:i') . ' - ' . $payment['payment_method_id'] . ' - ' . $payment['transaction_details']['net_received_amount']);
                break;    
            case 'in_metiation':
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mercadopago2_order_status_id_in_mediation'), date('d/m/Y h:i') . ' - '. $payment['payment_method_id'] . ' - ' . $payment['transaction_details']['net_received_amount']);
                break;
            default:
                $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('mercadopago2_order_status_id_pending'), date('d/m/Y h:i') . ' - ' . $payment['payment_method_id'] . ' - ' . $payment['transaction_details']['net_received_amount']);
                break;
    ...
    public function retorno() 
    {
        if (isset($this->request->get['collection_id'])) {
            ...
            foreach ($ids as $id) {
                error_log('processando id ' . $id);
                $resposta = $mp->get_payment_info($id);
                error_log('resposta do retorno');
                error_log($resposta);
                ...
                error_log("ID: " . $id);
                error_log("Status: " . json_encode($order_status));
